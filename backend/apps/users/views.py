...
class GoogleOAuthCallbackView(APIView):
    """
    Handle Google OAuth callback (US-006).

    GET /api/v1/auth/google/callback/
    Processes OAuth response from Google and handles account creation/linking.
    """
    permission_classes = [AllowAny]

    def get(self, request):
        """Process Google OAuth callback and create/link account."""
        ...
        try:
            # Complete the OAuth flow using allauth
            ...
            # Process the callback to get user info
            # This will call allauth's internal callback handling
            app = provider.get_app(request)

            # Get access token from callback
            code = request.GET.get('code')
            if not code:
                error = request.GET.get('error', 'invalid_request')
                frontend_url = settings.SITE_URL
                return HttpResponseRedirect(f"{frontend_url}/auth/callback?error={error}")

            # Complete the social login flow
            # allauth will fetch user profile data from Google
            login = adapter.complete_login(request, app, token=None, response=None)
            login.state = SocialLogin.state_from_request(request)

            # Extract user info from login
            email = login.account.extra_data.get('email')
            first_name = login.account.extra_data.get('given_name', '')
            last_name = login.account.extra_data.get('family_name', '')
            avatar_url = login.account.extra_data.get('picture', '')
            ...
            try:
                ...
                if social_account:
                    # User exists and Google is already linked - just login
                    ...
                    # User exists but Google is not linked - require password confirmation
                    ...
                    return HttpResponseRedirect(
                        f"{frontend_url}/auth/callback?link_required=true&provider=google"
                    )

            except User.DoesNotExist:
                # Email is new - create user and social account
            ...
            frontend_url = settings.SITE_URL
            return HttpResponseRedirect(
                f"{frontend_url}/auth/callback?error=provider_error&provider=google"
            )
