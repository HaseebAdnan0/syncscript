...
class PasswordResetFlowTests(TestCase):
    ...
    def test_password_reset_request_queues_email_task(self, mock_reset_task):
        """Test that password reset request queues async email task with correct parameters."""
        data = {'email': 'resettest@example.com'}
        response = self.client.post(self.password_reset_url, data, format='json', REMOTE_ADDR='192.168.1.100')

        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Verify Celery task was called with correct user ID
        mock_reset_task.assert_called_once()
        call_args = mock_reset_task.call_args
        self.assertEqual(call_args[0][0], self.user.id)  # First arg is user_id
        # Second arg is uid (base64 encoded user pk)
        # Third arg is token (generated by PasswordResetTokenGenerator)
        # Fourth arg is request_ip
        self.assertEqual(call_args[0][3], '192.168.1.100')  # Fourth arg is request_ip
