...
class PusherIntegrationTestCase(TestCase):
    ...
    @override_settings(PUSHER_APP_ID='test_app_id')
    @patch('apps.notifications.services.pusher_client')
    def test_pusher_triggers_on_notification_creation(self, mock_pusher: MagicMock) -> None:
    ...
    @override_settings(PUSHER_APP_ID='test_app_id')
    @patch('apps.notifications.services.pusher_client')
    def test_badge_update_reflects_unread_count(self, mock_pusher: MagicMock) -> None:
    ...
    @override_settings(PUSHER_APP_ID='test_app_id')
    @patch('apps.notifications.services.pusher_client')
    def test_badge_count_excludes_read_notifications(self, mock_pusher: MagicMock) -> None:
    ...
    @override_settings(PUSHER_APP_ID='')
    @patch('apps.notifications.services.pusher_client')
    def test_pusher_skipped_when_not_configured(self, mock_pusher: MagicMock) -> None:
        ...
        # But Pusher should not be triggered
        mock_pusher.trigger.assert_not_called()

    @override_settings(PUSHER_APP_ID='test_app_id')
    @patch('apps.notifications.services.pusher_client')
    def test_pusher_failure_does_not_block_notification(self, mock_pusher: MagicMock) -> None:
        """Test notification is created even if Pusher fails."""
        # Make Pusher trigger raise exception
    ...
    @override_settings(PUSHER_APP_ID='test_app_id')
    @patch('apps.notifications.services.pusher_client')
    def test_pusher_uses_private_user_channel(self, mock_pusher: MagicMock) -> None:
